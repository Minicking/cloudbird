...
class Email:
    def __init__(self, config):
        try:
            ...
            self.smtp.login(user=config.email_sender,
    ...
    def _thread_send(self, data, Type):
        if Type == 0:
            msg = MIMEText(
                "您接收到了来自云鸟的注册验证码<div style='color:red;font-size:15px;display:inline'>%s</div>，请于5分钟内使用,过期失效。"
                % (data['confirm']),
                'HTML',
                _charset="utf8")
        msg["Subject"] = "云鸟"
        ...
        msg["to"] = data['target']
        self.smtp.sendmail(from_addr=self.sender,
                           to_addrs=data['target'],
                           msg=msg.as_string())
