...
class HTTPProcess:
    ...
    def createConfirmCode(self):
        return "".join(random.sample('zyxwvutsrqponmlkjihgfedcba1234567890',
                                     5))
    ...
    def process(self):
        ...
        if self.interface == 'getconfirmcode':
            ...
            if self.checkRequestCondition('getconfirmcode'):
                print('[3]数据合理性检查通过')
                if self.header['confirm'] == self.getRightConfirm(
                        self.parameter['email']):
                    print('[4]身份码合法,开始进行验证码入库操作')
                    confirmcode = self.createConfirmCode()
                    existID = self.db.exist('confirmcode',
                    ...
                    if index > 0:
                        ...
                        print('[%d]发送验证码邮件:' % index, self.parameter['email'],
                              confirmcode)
                        self.sendConfirmCodeEmail(self.parameter['email'],
                                                  confirmcode)
                else:
            ...
            if self.checkRequestCondition('register'):
                ...
                if self.db.exist('user',
                    ...
                    if self.db.exist('user',
                        ...
                        print('[4]账号或邮箱可被使用')
                        if self.db.exist('confirmcode',
                                         {'email': self.parameter['email']}):
                            print('[5]邮箱验证码已在数据库内')
                            count, r = self.db.selectFromWhere(
                                'confirmcode',
                                'email=\'%s\'' % (self.parameter['email']))
                            code = r[0][2]
...
class HTTPServer:
    ...
    def start(self):
        ...
        while True:
            ...
            clientProcess = threading.Thread(target=self.clientProcess,
                                             args=(client_socket, ))
            clientProcess.start()
